...
bool Visualizer::createMarkers(const std_msgs::Header& header, long ID,
    ...
    if (object_class_to_marker_map_[map_key].show_text) {
        ...
        text_marker.pose.position.z += ev_pose_TRANSFORMED.getOrigin().getZ()-10;
        text_marker.header.stamp = header.stamp;
        text_marker.id = 3 * ID + 1;
        text_marker.scale.x=10;
        text_marker.scale.y=10;
        text_marker.scale.z=10;
        text_marker.color.r=color_mapping_["gold"].r;
        text_marker.color.g=color_mapping_["gold"].g;
        text_marker.color.b=color_mapping_["gold"].b;
